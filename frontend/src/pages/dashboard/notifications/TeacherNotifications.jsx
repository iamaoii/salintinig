...
export default function TeacherNotifications() {
  ...
  const fetchNotifications = async (isInitial = false) => {
    try {
      ...
      console.warn('Failed to fetch teacher notifications:', err);
    } finally {
  ...
  const filteredNotifications = useMemo(() => {
    ...
    if (filter === 'Assessments') return notifications.filter((n) => n.notification_type === 'reading_level' || n.notification_type === 'assessment');
    if (filter === 'Class Activities') return notifications.filter((n) => n.notification_type === 'activity' || n.notification_type === 'phil_iri');
    if (filter === 'Announcements') return notifications.filter((n) => n.notification_type === 'announcement' || n.notification_type === 'general' || n.notification_type === 'system');
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <div>
          <BackButton to="/teacher/overview" label="Back to Overview" />
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-3xl font-bold text-ink">Notifications & Activity Alerts</h1>
            </div>
            <p className="mt-1 text-xs text-ink/50">
              Stay updated on student Phil-IRI oral reading assessments, class progress alerts, and school announcements
            </p>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-ink/10 bg-cream p-4 shadow-[0px_4px_8px_0px_rgba(26,24,22,0.04)]">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center rounded-full border border-ink/10 bg-ink/5 p-0.5 text-xs font-semibold overflow-x-auto">
              {['All', 'Unread', 'Assessments', 'Class Activities', 'Announcements'].map((cat) => (
                <button
                  key={cat}
                  type="button"
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-1 text-xs text-ink/50 max-w-sm leading-relaxed">
                There are currently no notifications or activity alerts matching your selected filter.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-full bg-ink/5 text-ink">
                      {n.notification_type === 'reading_level' || n.notification_type === 'assessment' ? (
                        ...
                        <FlagPennant size={18} className="text-emerald-600" />
                      ) : n.notification_type === 'phil_iri' ? (
                        <Article size={18} className="text-amber-600" />
                      ) : (
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded bg-ink/5 px-1.5 py-0.2 text-[9px] font-bold text-ink/60 uppercase">
                          {n.notification_type === 'system' ? 'NOTICE' : n.notification_type || 'ALERT'}
                        </span>
                      </div>
