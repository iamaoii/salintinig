 ...
 */
async function transcribeAudio(audioFilePath, language = 'tl', originalFilename = '', passageText = '') {
  ...
  // Extract base words from passage to suppress Whisper's grammatical inflection prior
  let baseTokens = '';
  ...
  // System prompt to instruct Whisper to retain exact acoustic pronunciations without autocorrecting
  const verbatimPrompt = langCode === 'tl'
  ...
  try {
    ...
    // Ensure multer extensionless temp files have an audio extension hint (.m4a/.wav) for Groq API
    let fileStream = fs.createReadStream(audioFilePath);
